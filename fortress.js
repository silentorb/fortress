...
var User_Content = (function (_super) {
    ...
    User_Content.is_open_query = function (query) {
        var filters = query.filters.filter(function (filter) { return filter.path == query.trellis.primary_key; });
        return filters.length == 0;
    };
    User_Content.prototype.limited_to_user = function (query, user) {
        var filters = query.filters.filter(function (filter) { return filter.path == 'user'; });
        if (filters.length !== 1)
            return false;
...
function run(user, test, core) {
    ...
    for (var i in test.trellises) {
        ...
        var trellis_gates = user_gates.filter(function (gate) { return trellis_test.is_possible_gate(gate); });
        ...
        for (var j in trellis_test.properties) {
            ...
            if (condition.is_implicit && result.is_blacklisted(condition))
                continue;
            var property_gates = trellis_gates.filter(function (gate) { return condition.is_possible_gate(gate, trellis_test); });
...
var Trellis_Condition = (function () {
    ...
    Trellis_Condition.prototype.fill_implicit = function () {
        var _this = this;
        if (this.properties) {
            var primary_keys = this.trellis.get_primary_keys().map(function (p) { return p.name; });
            var properties = MetaHub.filter(this.trellis.get_all_properties(), function (p) { return primary_keys.indexOf(p.name) == -1; });
            this.properties = MetaHub.map(properties, function (p) { return new Property_Condition(p, _this.actions, true); });
        }
    };
...
var Property_Condition = (function () {
    function Property_Condition(property, actions, is_implicit) {
        if (is_implicit === void 0) { is_implicit = false; }
...
var Result = (function () {
    ...
    Result.prototype.get_message = function () {
        return "You are not authorized to perform this request: \n" + this.walls.map(function (wall) { return wall.get_message(); }).join("\n");
    };
...
var Core = (function () {
    ...
    Core.prototype.post_process_result = function (result) {
        ...
        var promises = result.post_actions.concat(function () { return result.finalize(); });
